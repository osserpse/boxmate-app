            ...
            {files.length > 0 && (
              <div className="grid grid-cols-4 gap-2">
                {files.map((uploadedFile, index) => (
                  <div key={`${uploadedFile.file.name}-${index}`} className="aspect-square bg-secondary rounded-lg relative group overflow-hidden">
                    <Image
                    ...
                    {uploadedFile.uploaded && (
                      <div className="absolute bottom-1 left-1 right-1 bg-success text-success-foreground text-xs text-center py-1 rounded">
                        Uppladdad
                      </div>
                    )}
                  </div>
                ...
                {files.length < 8 && (
                  <div
                    onClick={handleFileInput}
                    className="aspect-square bg-secondary border-2 border-dashed border-border rounded-lg flex items-center justify-center cursor-pointer hover:border-primary/60 transition-colors"
                  >
